...
function main()
    ...
    function uniformcircularmotionfigure()
        ...
        rectangle(ax, 'position', [pl(1, end) - circleRadius, pl(2, end) - circleRadius, circleDiameter, circleDiameter], 'Curvature', [1, 1], 'FaceColor', [0, 0, 0]);
        rectangle(ax, 'position', [pl(1, end) - circleRadius, pl(2, end) - circleRadius, circleDiameter, circleDiameter], 'Curvature', [1, 1], 'FaceColor', [0, 0, 0]); % added due to rendering issues
        arrow(ax, O, proj_pl_end_pl_1);
        arrow(ax, proj_pl_end_pl_1, pl(:, end) - circleRadius * normalize(pl(:, end) - proj_pl_end_pl_1, "norm", 2));
        arrow(ax, O, pl(:, end) - circleRadius * normalize(pl(:, end) - O, "norm", 2));
        axis(ax, 'equal', 'off');
